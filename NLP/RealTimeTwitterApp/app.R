...
ui <- dashboardPagePlus(
  ...
  dashboardBody(
    ...
    fluidRow(),
    fluidRow(
      img(src = 'https://i.ibb.co/6DyL7N4/Emotion-People-Cover-Desperate-Cry-Desperately-314481.jpg', align = "right") ,
      boxPlus(
        ...
        p(
          textInput(
            inputId = "twitter_input",
            label = "I am curious what people think about...",
            value = "#Warsaw"
          ),
          sliderInput(
            "obs",
            "Number of Twitts to analyse:",
            min = 0,
            max = 1000,
            value = 200
          )
    ...
    fluidRow(
      boxPlus(
        ...
        dropdown_menu = dropdownItemList(),
        p(
          "If you encoutered error while running the app, it unfortunately means that standard Twitter's API rate limits are exceeded. Please wait a bit (max 15 minutes) and try again. Read more: ",
    ...
    fluidRow(
      box(
        title = "Languages Of User Accounts",
        width = 6,
        background = "blue",
        withSpinner(
          plotlyOutput("language"),
          type = getOption("spinner.type", default = 6),
          color = getOption("spinner.color", default = "#ffffff")
        )
      ),
      box(
        title = "Wordcloud - Most Common Words",
        width = 6,
        background = "blue",
        withSpinner(
    ...
    fluidRow(
      ...
      box(
        title = "The Most Common Word's Associations",
        width = 6,
        background = "red",
        withSpinner(
          plotOutput("assocs"),
    ...
    fluidRow(
      ...
      box(
        title = "What are the emotions?",
        width = 6,
        background = "blue",
        withSpinner(
          plotlyOutput("emotion"),
          type = getOption("spinner.type", default = 6),
          color = getOption("spinner.color", default = "#ffffff")
        )
      )
    ...
    fluidRow(
      ...
      Write me on",
        tags$a(href = "mailto:magdalenekortas@gmail.com", "magdalenekortas@gmail.com"),
        "or find me on",
        tags$a(href = "https://www.linkedin.com/in/mkortas/", "Linkedin."),
        " The app was created for The 2019 WIA Data Viz competition on Women in Analytics conference. "
      )
    )
  )
)
...
server <- function(input, output, session) {
  data_loading <- function() {
    ...
    top1 <<- head(top, n = 3)
    account_lang <<-
      found_tweets %>% group_by(account_lang) %>% count() %>% arrange(desc(n)) %>% head(10)
