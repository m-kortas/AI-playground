...
data = pd.merge(geo_data, merged, how='left', on=[
    'city']).dropna(subset=['geometry'])
...
def plot_map(column, title, low, high, text):
    ...
    color_mapper = LinearColorMapper(
        palette=palette, low=low, high=high, nan_color='#ffffff')
    color_bar = ColorBar(color_mapper=color_mapper, label_standoff=8, width=500, height=20,
                         border_line_color=None, location=(0, 0), orientation='horizontal',
                         major_label_overrides=tick_labels)
    p = figure(title=title, plot_height=500, plot_width=350,
